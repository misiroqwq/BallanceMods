...
void NewSpawn::OnLoad() {
    ...
    reset_orientation_enabled = GetConfig()->GetProperty("Main", "Random_Orientation");
    reset_orientation_enabled->SetDefaultBoolean(true);
    reset_orientation_enabled->SetComment("Make the ball face a random direction upon reset.");
    orientation_enabled = reset_orientation_enabled->GetBoolean();
...
void NewSpawn::OnProcess() {
    ...
    if (input_manager->IsKeyPressed(key_prop) && Ball_Active) {
        ...
        m_BML->AddTimer(2ul, [this, current_ball, mm, ballDeact]() {
            ...
            // [transport] orientation random
            if (orientation_enabled) {
                VxVector randomDir(float(rand() % 2000 - 1000), float(rand() % 2000 - 1000), float(rand() % 2000 - 1000));
                VxVector randomUp(float(rand() % 2000 - 1000), float(rand() % 2000 - 1000), float(rand() % 2000 - 1000));
                randomDir.Normalize();
                randomUp.Normalize();
                current_ball->SetOrientation(&randomDir, &randomUp);
            }
